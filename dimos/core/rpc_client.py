...
class AsyncSpecProxy:
    ...
    A consumer that types ``ref: SomeSpec`` where ``SomeSpec`` declares
    ``async def foo`` will see ``self.ref.foo(x)`` return an awaitable. The
    underlying RPC call is still synchronous over the wire, so we offload it
    to a thread executor — the caller's event loop stays unblocked while the
    response round-trips. Sync-declared methods on the spec pass straight
    through to the inner proxy unchanged.

    Picklable so ``set_module_ref`` can ship it across to the worker process.
    """
