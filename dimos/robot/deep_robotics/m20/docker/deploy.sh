...
case "${CMD}" in
    push)
        ...
        DOCKER_BUILDKIT=1 docker build \
            ...
            -f "${DIMOS_ROOT}/dimos/robot/deep_robotics/m20/docker/Dockerfile.nav" \
            "${DIMOS_ROOT}"
    ...
    dev)
        ...
        remote_ssh docker cp ${DEPLOY_DIR}/dimos/robot/deep_robotics/m20/docker/launch_nos.py ${CONTAINER_NAME}:/opt/dimos/launch_nos.py
        remote_ssh docker cp ${DEPLOY_DIR}/dimos/robot/deep_robotics/m20/docker/entrypoint.sh ${CONTAINER_NAME}:/opt/dimos/entrypoint.sh
        remote_ssh docker cp ${DEPLOY_DIR}/dimos/robot/deep_robotics/m20/docker/fastdds.xml ${CONTAINER_NAME}:/opt/dimos/docker/fastdds.xml
    ...
    bridge-build)
        ...
        rsync -avz --delete \
            -e "ssh ${SSH_OPTS}" \
            "${DIMOS_ROOT}/dimos/robot/deep_robotics/m20/docker/drdds_bridge/" \
    ...
    bridge-deploy)
        ...
        rsync -avz \
            -e "ssh ${SSH_OPTS}" \
            "${DIMOS_ROOT}/dimos/robot/deep_robotics/m20/docker/drdds_bridge/drdds-bridge.service" \
