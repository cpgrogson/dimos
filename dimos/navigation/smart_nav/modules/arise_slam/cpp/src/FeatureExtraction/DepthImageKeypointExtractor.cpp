...
void DepthKeypointExtractor::featureExtraction(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pc_in, pcl::PointCloud<pcl::PointXYZ>::Ptr& pc_out_edge, pcl::PointCloud<pcl::PointXYZ>::Ptr& pc_out_surf){
    ...
    // printf("[arise_slam] input feature %d\n", (int)pc_in->points.size());
    ...
    for (int i = 0; i < (int) pc_in->points.size(); i++)
    {
        ...
        // printf("[arise_slam] last_angle %lf, angle %lf, diff %f\n", last_angle, angle, fabs(angle - last_angle));
    ...
    // printf("[arise_slam] total points array %zu\n", laserCloudScans.size());
